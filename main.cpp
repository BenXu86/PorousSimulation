...
void draw2D ()
{
	...
	if ( bHelp ) {	
		...
		//multi fluid
		float st = psys.GetParam(PTIME_INSERT) + psys.GetParam(PTIME_PRESS)+psys.GetParam(PTIME_FORCE) + psys.GetParam(PTIME_ADVANCE) + psys.GetParam(PTIMEDRIFTVEL) + psys.GetParam(PTIMEALPHA) + psys.GetParam(PTIMECORR);
		...
		sprintf ( disp, "GPU Free Memory:     %d bytes", fm); drawText(20, 310, disp);
		sprintf ( disp, "GPU Total Memory:    %d bytes", tm); drawText(20, 320, disp);
		//multi fluid
...
//	iso = sin(frame*0.01f );
	...
	else				glClearColor ( 0.1, 0.1, 0.1, 1 );
...
void keyboard_func ( unsigned char key, int x, int y )
{
	...
	case 'B': case 'b':		
		if (psys.GetYan(START_OUTPUT) == 0)
		{
			psys.SetYan	(OUTPUT_INT,OUT_INT_FRAME );
			psys.SetYan (START_OUTPUT,1);
		}
		else
			psys.SetYan	(START_OUTPUT,0);
		break;
	case '7':	
		if (psys.GetYan(SHOW_BOUND) <= 2)
		{
			psys.SetYan (SHOW_BOUND,psys.GetYan(SHOW_BOUND)+1);
		}
		else
			psys.SetYan	(SHOW_BOUND,0);
		break;
	case '8':
...
void mouse_drag_func ( int x, int y )
{
	...
	case MODE_CAM:
		if ( dragging == DRAG_LEFT ) {
			cam.moveOrbit ( -dx*0.5, dy*0.5, 0, 0 );
		} else if ( dragging == DRAG_RIGHT ) {
			cam.moveOrbit ( 0, 0, 0, dy*0.35 );
		} 
		else if (dragging == DRAG_UP)
		{
			cam.moveOrbit(0,0,0,1.5);
		}
		else if (dragging == DRAG_DOWN)
		{
			cam.moveOrbit(0,0,0,-1.5);
		}
		break;
