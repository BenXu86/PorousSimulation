...
double a[18],scaleP,scaleP3,scaledis,b[6];
float cont[3];
int loadwhich;
...
//#ifdef MULTIFLUID
//	CopyMfFromCUDA ( m_alpha, m_alpha_pre, m_pressure_modify, (float*) m_vel_phrel, m_restMass, m_restDensity, m_visc, (float*)m_velxcor, (float*)m_alphagrad);
//#endif
#ifdef NEW_BOUND
...
void FluidSystem::Setup ( bool bStart )
{
	...
	SetupGridAllocate ( m_Vec[PVOLMIN], m_Vec[PVOLMAX], m_Param[PSIMSCALE], m_Param[PGRIDSIZE], 1.0 );	// Setup grid
		...
		FluidMfParamCUDA (m_fluidDensity,m_fluidVisc,m_fluidDiffusion,m_Param[FLUID_CATNUM], m_DT, m_splitVolume, m_mergeVolume,cont[0],cont[1],cont[2]);
#else
...
#ifdef MULTIFLUID
	case RUN_MULTI_CUDA_FULL: RunSimulateMultiCUDAFull(); break;
#endif
	};
...
void FluidSystem::PackParticles ()
{
	...
	for (int c=0; c < m_GridTotal; c++) {
		...
		while ( j != -1 ) {
			...
			*(int*) dat =		*(mIsBound+j);		dat += sizeof(int);
			dat += sizeof(int);
			j = *(mGridNext+j);
			cnt++;
		}
...
//		printf("%d\n",outP);
	...
	if ((GetYan(START_OUTPUT) == 1))
	{
		if ((m_Frame % GetYan(OUTPUT_INT)==0))
			outputFile();
	}
	//-------------------------------------- DEBUGGING
...
void FluidSystem::ParseXML_BI2 (std::string name)
{
	xml.setBase(name,0);
	xml.assignValueStr(Bi2Dir,"Dir");
}

void FluidSystem::ParseXML_Bound (std::string name)
{
#ifdef NEW_BOUND
	xml.setBase ( name, 5 );
...
void FluidSystem::SetupSpacing ()
{
	...
	// Particle Boundaries
#ifdef MULTIFLUID
	m_Vec[PBOUNDMIN].Set(b[0],b[1],b[2]);
	m_Vec[PBOUNDMAX].Set(b[3],b[4],b[5]);
#else
...
#ifdef MULTIFLUID
void FluidSystem::SetupMfAddVolume ( Vector3DF min, Vector3DF max, float spacing, Vector3DF offs, int cat )
{
	Vector3DF pos;
	int n = 0, p;
	float dx, dy, dz, x, y, z;
	...
	if(cat >= m_Param [ FLUID_CATNUM ])return;
...
void FluidSystem::RunSimulateMultiCUDAFull ()
{
	...
	//	MfMergeCUDA();
	//}
	//record ( PTIMESPLIT, "Split CUDA", start );
	...
	//	SortGridCUDA( 0x0 );
	//	CountingSortFullCUDA_( 0x0 );
	//	MfComputeDensityCUDA();
	//	MfSplitCUDA ();	
	//	mNumPoints = MfGetPnum();
	//}
	//record ( PTIMESPLIT, "Split CUDA", start );
}
...
#ifdef NEW_BOUND
	ParseXML_Bound("BoundInfo");
	//	ParseXML_BI2 ( "BI2" );
	Bi2Dir = ".\\BoundaryOut\\BoundaryOut\\Boundary.bi2";
	BI2Reader bi2reader(Bi2Dir);
	bi2reader.GetInfo(false);
	bi2reader.PrintInfo();

//	Bi2Dir1 = ".\\BoundaryOut\\BoundaryOut\\BoundaryMove.bi2";
//	BI2Reader bi2reader1(Bi2Dir1);
//	bi2reader1.GetInfo(false);
//	bi2reader1.PrintInfo();
#endif

	Vector3DF pos;
	Vector3DF min, max;
	
	m_Vec [ PVOLMIN ].Set ( -100, -40, -100 );
	m_Vec [ PVOLMAX ].Set (100, 200, 100 );
		
	m_Param [PMASS] = 0.00002567875;				
	m_Param [PRADIUS ] = 0.01;
	m_Param [PNUM] = 65536 * 2;



	//m_Param[PFORCE_MIN] = 20.0;      //Test
	m_Param [PINTSTIFF] = 1.5;

	m_Param [FLUID_CATNUM] = 3;
	m_fluidPMass[0] = 0.00002567875; m_fluidPMass[1] = 0.00002567875*2; m_fluidPMass[2] = 0.00002567875*3;
	m_fluidDensity[0] = 600.0; m_fluidDensity[1] = 600.0*2; m_fluidDensity[2] = 600.0*3;
	m_fluidVisc[0] = m_fluidVisc[1] = m_fluidVisc[2] = m_Param[PVISC];

	ParseXML ( "MultiScene", 1, true );
	...
	for (int i = 0;i<18;i++)
		fscanf(fp,"%lf",&a[i]);
	fscanf(fp,"%lf",&scaleP);
	fscanf(fp,"%lf",&scaledis);
	for (int i = 0;i<6;i++)
		fscanf(fp,"%lf",&b[i]);
	fscanf(fp,"%lf",&m_Param[PFORCE_FREQ]);
	fscanf(fp,"%d",&loadwhich);
	fscanf(fp,"%f%f%f",&cont[0],&cont[1],&cont[2]);
	fclose(fp);
...
#ifdef NEW_BOUND
	m_Param[PNUM] += bi2reader.info.nbound;

//	m_Param[PNUM] += bi2reader1.info.nbound;
#endif
	m_Param [PGRIDSIZE] = 2*m_Param[PSMOOTHRADIUS] / m_Param[PGRID_DENSITY];
	m_fluidPMass[0] = m_Param[PMASS]; m_fluidPMass[1] = m_Param[PMASS]*2; m_fluidPMass[2] = m_Param[PMASS]*3;

	m_splitVolume = m_fluidPMass[0]/m_fluidDensity[0] * 4;
	m_mergeVolume = m_fluidPMass[0]/m_fluidDensity[0] / 4;
	...
	if (loadwhich == 0)
	{
		...
		m_Vec [ PINITMIN ].Set (a[0],a[1],a[2]);
		m_Vec [ PINITMAX ].Set (a[3],a[4],a[5]);
		//m_Vec [ PINITMIN ].Set (15,15,15);
		//m_Vec [ PINITMAX ].Set (45,85,35);
		mMaxPoints = 32768*scaleP;
		SetupMfAddVolume ( m_Vec[PINITMIN], m_Vec[PINITMAX], m_Param[PSPACING], Vector3DF(0,0.1,0),0);

		m_Vec [ PINITMIN ].Set (a[6],a[7],a[8]);
		m_Vec [ PINITMAX ].Set (a[9],a[10],a[11]);
		//m_Vec [ PINITMIN ].Set (15,15,15);
		//m_Vec [ PINITMAX ].Set (45,85,35);
		...
		m_Vec [ PINITMIN ].Set (a[12],a[13],a[14]);
		m_Vec [ PINITMAX ].Set (a[15],a[16],a[17]);
		//m_Vec [ PINITMIN ].Set (-45,15,25);
		//m_Vec [ PINITMAX ].Set (-15,85,45);

		mMaxPoints += 65536*scaleP;
		SetupMfAddVolume ( m_Vec[PINITMIN], m_Vec[PINITMAX], m_Param[PSPACING], Vector3DF(0,0.1,0),2);
	}
	else
		mMaxPoints += loadParticle("save_stat.txt");
#ifdef NEW_BOUND
	mMaxPoints += bi2reader.info.nbound;
	SetupAddBound(bi2reader,1);

//	mMaxPoints += bi2reader1.info.nbound;
//	SetupAddBound(bi2reader1,2);
#endif
